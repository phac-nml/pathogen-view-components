...
module Pathogen
  ...
  module ButtonToHelper
    ...
    #   pathogen_button_to "Delete", post_path(@post), method: :delete, tone: :danger, emphasis: :soft
    def pathogen_button_to(name = nil, options = nil, html_options = nil, &block)
      ...
      submit_button.with_content(capture(&block)) if block
      rails_html_options = build_rails_button_to_options(form_extra, method_override, submit_button)

      button_to(options, rails_html_options) { tag.span(submit_button.button_text) }
    end
