...
module Pathogen
  ...
  class DataGridComponentTest < ViewComponentTestCase
    test 'renders grid with sticky columns' do
    ...
    test 'uses default aria-label when no aria attribute is passed' do
      render_inline(Pathogen::DataGridComponent.new(
                      sticky_columns: 0,
                      rows: [
                        { id: 'S-001', name: 'Sample one' }
    ...
    test 'accepts custom aria-label via system arguments' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: [{ id: 'S-001' }],
      ...
      assert_selector '.pathogen-data-grid__grid[aria-label="Sample inventory"]'
    end

    test 'accepts aria-labelledby via system arguments' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: [{ id: 'S-001' }],
      ...
      assert_selector '.pathogen-data-grid__grid[aria-labelledby="my-heading"]'
      assert_no_selector '.pathogen-data-grid__grid[aria-label]'
    end
    ...
    test 'renders div-based grid with ARIA roles and no table elements' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: [
                        { id: 'S-001', name: 'Sample one' },
                        { id: 'S-002', name: 'Sample two' }
      ...
      assert_selector 'div[role="row"]', count: 3
      assert_selector 'div[role="columnheader"]', count: 2
      assert_selector 'div[role="gridcell"]', count: 4
    end

    test 'sets aria-rowcount and aria-colcount on the grid' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: [
                        { id: 'S-001', name: 'Alpha' },
                        { id: 'S-002', name: 'Beta' },
      ...
      assert_selector 'div[role="grid"][aria-rowcount="4"]'
      assert_selector 'div[role="grid"][aria-colcount="2"]'
    end

    test 'sets aria-rowindex on each row' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: [
                        { id: 'S-001', name: 'Alpha' },
                        { id: 'S-002', name: 'Beta' }
    ...
    test 'cell data attributes include target, row/column index, and interactive flag' do
      render_inline(Pathogen::DataGridComponent.new(
    ...
    test 'renders viewport and spacer for scroll virtualization' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: [
                        { id: 'S-001', name: 'Alpha' },
                        { id: 'S-002', name: 'Beta' }
    ...
    test 'empty rows without empty_state slot renders grid with header only' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: []
                    )) do |grid|
        grid.with_column('ID', key: :id)
    ...
    test 'uses grid-template-columns style from column widths' do
      render_inline(Pathogen::DataGridComponent.new(
                      rows: [{ id: 'S-001', name: 'Alpha' }]
                    )) do |grid|
        grid.with_column('ID', key: :id, width: 120)
